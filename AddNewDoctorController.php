...
if (isset($_POST['clicked'])) {
    ...
    $specialtyH = mysqli_real_escape_string($db, $_POST['Specialty']);

    $specialtyIdSQL = "SELECT specialty_id FROM specialties WHERE specialty_name='$specialtyH'";
    $result = mysqli_query($db, $specialtyIdSQL);
    $specialtyM = mysqli_fetch_assoc($result);
    $specialty = $specialtyM['specialty_id'];
    ...
    if (count($errors) == 0) {
        ...
        $query = "INSERT INTO users (username,first_name,last_name, email, password,registration_date,user_role,specialty_id) 
  			  VALUES('$username','$firstName','$lastName', '$email', '$password','$registration_date','$user_role','$specialty')";
